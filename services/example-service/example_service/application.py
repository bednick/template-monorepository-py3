...
from example_service import config, dependencies, routers
...
async def generic_exception_handler(request: fastapi.Request, call_next) -> fastapi.responses.JSONResponse:
    """
    Если ловить все ошибки средствами exception_handler, то в логи сохраняется "Exception in ASGI application"
    https://github.com/tiangolo/fastapi/issues/4025
    """
    try:
        return await call_next(request)
    except Exception as exc:
        method, path, status_code = request.scope["method"], request.scope["path"], 500
        logger.exception(f"HTTP error {method=} {path=} {status_code=}: {repr(exc)}")
        return fastapi.responses.JSONResponse(
            status_code=status_code, content={"message": f"{exc}", "detail": "Internal Server Error"}
        )

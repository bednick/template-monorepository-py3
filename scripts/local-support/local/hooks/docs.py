...
def run(updated_files: List[str], format_: str, filename: str) -> bool:
    ...
    apps = {
        updated_service: f"{utils.gen_module_name(updated_service)}.routers:app" for updated_service in updated_services
    }

    updated: bool = False
    for project_name, app in apps.items():
        try:
            ...
            if new_docs_filename:
                print(f"update docs file: {new_docs_filename}")
                updated = True

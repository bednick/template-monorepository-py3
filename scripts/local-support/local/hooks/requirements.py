...
def read_requirements(file: pathlib.Path) -> Set[str]:
    with open(file, "r", encoding="utf-8") as fp:
        data = fp.read()
    lines = data.split("\n")
    results = set()
    for line in lines:
        result = line.split("#")[0].strip().lower()
        if result:
            results.add(result)
    return results
...
def run(updated_filenames: List[str], root_filename: str) -> int:
    ...
    root_requirements = read_requirements(root_file)

    result = 0
    for updated_file in updated_files:
        updated_file_requirements = read_requirements(updated_file)
...
def configure_parser(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--root-filename", type=str, default="requirements.txt")
    return parser


def main():
    parser = configure_parser(argparse.ArgumentParser(description="Check requirements.txt"))
